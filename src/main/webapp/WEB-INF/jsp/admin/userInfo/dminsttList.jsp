...
<script type="text/javascript">
	...
	$(function(){
		...
		$('#result').on('click', '.btnView', function(e){
			$('#frm').attr('action', 'dminsttForm.do');
		...
		$('#frm').ajaxCwma({
			beforeSubmit:function(){
				...
				if($('[name="corpNo"]:eq(1)').val() && !$('[name="corpNo"]:eq(0)').val()){
					...
					$('[name="dminsttInfoVO.corpNo"]').val($('[name="corpNo"]:eq(0)').val()+'-'+$('[name="corpNo"]:eq(1)').val());
			...
			success:function(res){
				...
				$(res.list).each(function(){
					...
					html += '<td class="txt_left pl20"><a href="#" class="btnView" data-sn="'+this.userId+'">'+this.dminsttInfoVO.dminsttNm+'</a></td>';
					html += '<td>'+this.userId+'</td>';
					html += '<td>'+this.dminsttInfoVO.ceoNm+'</td>';
					html += '<td>'+this.bizno+'</td>';
					html += '<td>'+this.dminsttInfoVO.corpNo+'</td>';
...
<body>
	<div class="search_wrap">
		<form action="" id="frm" method="post">
			...
			<input type="hidden" name="se" value="USSE0004" />
			<input type="hidden" name="dminsttInfoVO.corpNo" value="" />
			
			<ul class="tab200" style="display:none">
				<li data-sttus="USST0001">수요기관회원</li>
			...
			<table class="tbl tbl_search">
				...
				<tbody>
					...
					<tr>
						...
						<th scope="row">기관명</th>
						<td><input type="text" class="w100p" name="dminsttInfoVO.corpNm" /></td>
					</tr>
					<tr>
