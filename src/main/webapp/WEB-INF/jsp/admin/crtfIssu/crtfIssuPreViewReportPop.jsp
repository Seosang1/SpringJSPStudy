...
<!--     	<meta http-equiv="X-UA-Compatible" content="IE=edge"/> -->
    ...
    <body>
		...
		let fileName = "grade/등급증명서발급신청서sign.ozr";
// 		let fileName = "grade/발급동의취소서.ozr";
		if(browser == "chrome"){
			function SetOZParamters_OZViewer(){
				...
				oz.sendToActionScript("connection.args1", "no=${reqstNo}");	//신청인 이름
				oz.sendToActionScript("export.format","pdf");
// 				oz.sendToActionScript("export.mode","normal");
// 				oz.sendToActionScript("export.confirmsave","true");
// 				oz.sendToActionScript("pdf.path","c:\\Temp\\");
//				oz.sendToActionScript("export.path","c:\\Temp\\");
				oz.sendToActionScript("eform.signpad_type", "dialog");
// 				oz.sendToActionScript("multiscreen.extraparam","tablet=wacom_wintab,tablet_size=1366/768,updatenow=true");
// 				oz.sendToActionScript("multiscreen.signpad_type","dialog");
// 				oz.sendToActionScript("eform.signpad_dialog_fullscreen", "viewer");          //그룹 서명
				oz.sendToActionScript("export.filename","기능등급증명서 발급신청서_"+'${brffcNm}');
				return true;
			}
			var opt = [];
// 			opt["mstyle"] = true;
			opt["signpad_dialog_fullscreen"] = "browser"; // browser, 서명 Dialog를 Viewer Div 영역 기준으로 띄울 경우
			start_ozjs("OZViewer","${ozServer }/oz60/ozhviewer/",opt);
			...
			function ZTInstallEndCommand_ZTransferX(param1,param2) { 
	            ...
	            Insert_OZViewer_Param("connection.reportname", fileName);
	            Insert_OZViewer_Param("connection.pcount","1");
	            Insert_OZViewer_Param("connection.args1","no=${reqstNo}");
        ...
        <div id= "InstallOZViewer">
            ...
            if(browser == "explorer"){
				...
				function savePDF() { //pdf 파일로 저장
					...
					OZReportViewer.ScriptEx("save","export.format=pdf;export.path=C:\\temp;export.mode=normal;export.filename="+"기능등급증명서 발급신청서_"+"${brffcNm}.pdf;export.confirmsave=false",";")
				}
