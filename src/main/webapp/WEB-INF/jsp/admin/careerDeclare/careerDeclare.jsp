...
<script type="text/javascript">
	...
	$(function(){
		...
		if($('[name="email"]').val()){
			$('[name="email1"]').val($('[name="email"]').val().split('@')[0]);
			$('[name="email2"]').val($('[name="email"]').val().split('@')[1]);
		}
		
		// 직종선택 selected
		<c:if test="${empty list.multiJssfcVO }">
	 		$('.jobList').each(function(i){
				var jsscOt = $(this).find('option');
				if(jsscOt.length == 1){
					$(this).parent().parent().find('[name="jssfcNoList"]').val(jsscOt.data('value'));
					$(this).parent().parent().find('.jobStr').val(jsscOt.val());
				
				}else if(jsscOt.length == 0){
					$(this).hide();
				}
			})
		</c:if>
		...
		// 직종선택 검색에 따른 이벤트
		$('.jobStr').on('input',function(){
			var jsscStr = $(this).val();
			
			if(!jsscStr.trim()){
				$(this).parent().parent().find('[name="jssfcNoList"]').val('');
				
			}else{
				var jsscOt = $(this).parent().parent().find('datalist option[value=' + jsscStr + ']');
				if(jsscOt.length == 1){
					$(this).parent().parent().find('[name="jssfcNoList"]').val(jsscOt.data('value'));
				}else{
					$(this).parent().parent().find('[name="jssfcNoList"]').val('');
		...
		$('#btnPrint').click(function(){
			if(!$('[name=snList]:checked').length){
				alert('경력인정신청 대상을 선택해주세요');
				return false;
			}
			
			var jssfcFlag = true;
			$('[name=snList]:checked').each(function(){
			 	if(!$(this).parent().parent().find('[name="jssfcNoList"]').val().trim()){
					alert('직종을 선택해주세요.');
					jssfcFlag = false;
				}
			});
		...
		$('#licenseChkbox').change(function(){
			if($(this).is(":checked")){
				$('#licenseChk').find('input:checkbox').each(function(){
		...
		$('#eduChkbox').change(function(){
			if($(this).is(":checked")){
				$('#eduChk').find('input:checkbox').each(function(){
					$(this).attr('checked', true);
				});
			}else{
				$('#eduChk').find('input:checkbox').each(function(){
		...
		$('#rewardChkbox').change(function(){
			if($(this).is(":checked")){
				$('#rewardChk').find('input:checkbox').each(function(){
					$(this).attr('checked', true);
				});
			}else{
				$('#rewardChk').find('input:checkbox').each(function(){
		...
		<table class="tbl tbl_form mt5">
			...
			<tbody>
				<tr>
					...
					<td>
						<input type="radio" id="rad201" name="se" value="CASE0001" title="구분" checked><label for="rad201">경력인정신청</label>
		...
		<table class="tbl tbl_form mt5">
			...
			<tbody>
				<tr>
					...
					<td colspan="3">
						<input type="text" class="w150" name="jumin1" title="주민등록번호 앞자리" maxlength="6" value="${param.jumin1}" required number>
						-
						<input type="text" class="w150" name="jumin2" title="주민등록번호 뒷자리" maxlength="7" value="${param.jumin2}" required number>
...
<!-- 							<option value="">유형선택</option> -->
				...
				<tr>
					...
					<th scope="row">이메일</th>
					<td>
						<input type="text" class="w20p" maxlength="30" name="email1" title="이메일 앞" value="" /> 
						@ <input type="text" class="w30p" maxlength="30" name="email2" title="이메일 뒤" value="" />
				...
				<tr>
					...
					<td colspan="3">
						<div class="input_date">
							<input type="text" style="width: 105px;" class="datepicker" name="rqstdt" title="신청일" value="${eo.rqstdt}" required>
	...
	if(str == "P") {		
		...
		<div id="catab_1" name="catab_1" style="display: block;">
			...
			<table class="tbl tbl_data mt5">
				...
				<thead>
					<tr>
						<th scope="col"><input type="checkbox" id="licenseChkbox" value="0"></th>
				...
				<tbody  id="licenseChk">
					...
					<tr>
						...
						<td>
							<input type="hidden" name="jssfcNoList">
							<input type="text" list="licenseJobList${sts.count }" class="jobStr" placeholder="직종 검색">
							<datalist id="licenseJobList${sts.count }" class="jobList">
								<c:forEach var="jssfcList" items="${list.multiJssfcVO }">
									<option data-value="${jssfcList.jssfcNo }" value="${jssfcList.jssfcNm }"></option>
								</c:forEach>
								<c:if test="${empty list.multiJssfcVO }">
									<option data-value="${list.stdrJssfcNo }" value="${list.stdrJssfcNm }">${list.stdrJssfcNm }</option>
								</c:if>
							</datalist>
						</td>
					</tr>
		...
		<div id="catab_2" name="catab_2" style="display: none;">
			...
			<table class="tbl tbl_data mt5">
				...
				<thead>
					<tr>
						<th scope="col"><input type="checkbox" id="eduChkbox" value="0"></th>
						<th scope="col"><span>과정명</span></th>
				...
				<c:forEach var="list" items="${eduList }" varStatus="sts">
					<tr>
						...
						<td>
							<input type="hidden" name="jssfcNoList">
							<input type="text" list="eduJobList${sts.count }" class="jobStr" placeholder="직종 검색">
							<datalist id="eduJobList${sts.count }" class="jobList">
								<c:forEach var="jssfcList" items="${list.multiJssfcVO }">
									<option data-value="${jssfcList.jssfcNo }" value="${jssfcList.jssfcNm }"></option>
								</c:forEach>
								<c:if test="${empty list.multiJssfcVO }">
		...
		<div id="catab_3" name="catab_3" style="display: none;">
			...
			<table class="tbl tbl_data mt5">
				...
				<thead>
					<tr>
						<th scope="col"><input type="checkbox" id="rewardChkbox" value="0"></th>
				...
				<tbody id="rewardChk">
					...
					<tr>
						...
						<td>
							<input type="hidden" name="jssfcNoList">
							<input type="text" list="rewardJobList${sts.count }" class="jobStr" placeholder="직종 검색">
							<datalist id="rewardJobList${sts.count }" class="jobList">
								<c:forEach var="jssfcList" items="${list.multiJssfcVO }">
									<option data-value="${jssfcList.jssfcNo }" value="${jssfcList.jssfcNm }"></option>
								</c:forEach>
								<c:if test="${empty list.multiJssfcVO }">
		...
		<table class="tbl tbl_search mt10">
			...
			<thead>
				<tr>
					<th scope="row" class=""><i class="fas fa-th-large mr10"></i>
						제출서류</th>
				</tr>
			</thead>
		</table>
