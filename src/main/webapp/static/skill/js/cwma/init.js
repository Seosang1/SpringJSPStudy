...
$(function(){
	...
	//form에 csrf토큰삽입
	$('form[method="post"]').append('<input type="hidden" name="csrfToken" value="'+csrfToken+'" />');
	
	//서브베너&location 설정
	if($('#sub-header nav a[href="'+uri+'"]')[0]){
		var txt = '';
		var menuList = $('#sub-header nav > ul > li:visible > a').clone();
		
		if($('#sub-header nav a[href="'+uri+'"]').parents('.drop-down')[0]){
			txt = $('#sub-header nav a[href="'+uri+'"]').parents('.drop-down').children('a').text().trim();
			$('#btnGroupDrop2').text($('#sub-header nav a[href="'+uri+'"]').not('.m-login-btn').not('.drop-down > a').text().trim());
			
			$($('#sub-header nav a[href="'+uri+'"]').parents('.drop-down').find('li a').clone()).each(function(){
				$(this).addClass('dropdown-item');
				$('[aria-labelledby="btnGroupDrop2"]').append(this);
				
				if(subBannerClass[uri.split('/')[2]] == 'mypage'){
					$('[aria-labelledby="btnGroupDrop2"] li').show();
					
					if(uri.indexOf('index.do') < 0){
						//퀵메뉴 움직
						$('.mypage-floating').show();
						$('.mypage-floating').hcSticky({stickTo: '.sub-section'});
						$('.mypage-floating').hcSticky('update', {top: 60});
					}
				}
			});
			
		}else{
			txt = $('#sub-header nav a[href="'+uri+'"]').not('.m-login-btn').not('.drop-down > a').text().trim();
			$('[role="group"].depth2').hide();
			
			if(subBannerClass[uri.split('/')[2]] == 'member')
				$('.location_wrap').hide();
		}
		
		$('.sub-banner-tlt h1').text(txt);
		$('#btnGroupDrop1').text(txt);
		
		if(subBannerClass[uri.split('/')[2]])
			$('.sub-header-inner').addClass(subBannerClass[uri.split('/')[2]]);
		
		$(menuList).each(function(){
			$(this).addClass('dropdown-item');
			$('[aria-labelledby="btnGroupDrop1"]').append(this);
		});
		
		$('#btnGroupDrop1').next().find('a').click(function(){
			$('#btnGroupDrop1').text($(this).text().trim());
			
			if($('#sub-header nav a:contains('+$(this).text()+')').parent().children('ul')[0]){
				$('[aria-labelledby="btnGroupDrop2"] a').remove();
				
				$($('#sub-header nav a:contains('+$(this).text()+')').parent().find('ul a').clone()).each(function(i){
					$(this).addClass('dropdown-item');
					$('[aria-labelledby="btnGroupDrop2"]').append(this);
					
					if(!i)
						$('#btnGroupDrop2').text($(this).text().trim());
				});
				$('[role="group"].depth2').show();
